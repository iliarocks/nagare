...
nonisolated enum ReorderProjection {
    ...
    static func applying<ID: Hashable, CollectionID: Hashable>(
        sources: [ID],
        to destinationCollection: CollectionID,
        before destination: ID?,
        in collections: [CollectionID: [ID]]
    ) throws -> [CollectionID: [ID]] {
        let sourceSet = Set(sources)
        guard sourceSet.count == sources.count else {
            throw ProjectionError.duplicateSource
        }

        let allValues = collections.values.flatMap { $0 }
        guard sources.allSatisfy({ source in
            allValues.count(where: { $0 == source }) == 1
        }) else {
            throw ProjectionError.missingSource
        }
        guard collections[destinationCollection] != nil else {
            throw ProjectionError.missingDestination
        }
        if let destination, sourceSet.contains(destination) {
            throw ProjectionError.destinationIsMovingItem
        ...
        var result = collections.mapValues { values in
            values.filter { !sourceSet.contains($0) }
        }
        guard var destinationValues = result[destinationCollection] else {
            throw ProjectionError.missingDestination
        }

        let insertionIndex: Int
        if let destination {
            guard let index = destinationValues.firstIndex(of: destination) else {
                throw ProjectionError.missingDestination
            }
            insertionIndex = index
        } else {
        ...
        destinationValues.insert(contentsOf: sources, at: insertionIndex)
        result[destinationCollection] = destinationValues
        return result
    }
}
